...
extension SearchParentView {
    @MainActor final class SearchViewModel: ObservableObject {
        ...
        func onSearchProgrammes(searchQuery: String) -> Void {
            self.status = .loading
            networkManager.get(.searchProgramme(searchQuery: searchQuery, schoolId: String(school!.id))) { (result: Result<Response.Search, NetworkError>) in
        ...
        fileprivate func fetchSchedule(programmeId: String, closure: @escaping () -> Void) -> Void {
            networkManager.get(.schedule(scheduleId: programmeId, schoolId: String(school!.id))) { (result: Result<Response.Schedule, NetworkError>) in
                DispatchQueue.main.async {
                    switch result {
                    case .success(let result):
